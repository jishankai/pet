...
class SocialController extends Controller
{
    ...
    public function actionFood($img_id, $alert_flag=0, $to='', $SID='')
   {
    ...
    public function actionStarIndex($star_id, $SID='')
    {
        ...
        $star['animals'] = Yii::app()->db->createCommand('SELECT a.aid, a.tx, a.name, SUM(i.stars) AS cnt FROM dc_image i LEFT JOIN dc_animal a ON a.aid=i.aid WHERE star_id=:star_id GROUP BY i.aid,a.aid,a.tx,a.name ORDER BY cnt DESC LIMIT 6')->bindValue(':star_id', $star_id)->queryAll();
        $star['images'] = Yii::app()->db->createCommand('SELECT i.img_id, i.url, i.stars, a.name FROM dc_image i LEFT JOIN dc_animal a ON i.aid=a.aid WHERE i.star_id=:star_id ORDER BY i.stars DESC LIMIT 30')->bindValue(':star_id', $star_id)->queryAll();
        
        $this->renderPartial('big_star_act', array('star'=>$star, 'SID'=>$SID));
    }
}
