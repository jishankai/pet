...
class StarController extends Controller
{
    ...
    public function actionContriApi($aid, $star_id)
    {
        ...
        if ($users_str!='') {
            $user_txs = Yii::app()->db->createCommand('SELECT usr_id, tx FROM dc_user WHERE usr_id IN (:users_str) ORDER BY FIELD(usr_id, :users_str)')->bindValue(':users_str', $users_str)->queryAll();
        } else {
            $user_txs = array();
        }
