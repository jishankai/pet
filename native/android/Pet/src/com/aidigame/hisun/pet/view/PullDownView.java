...
import com.aidigame.hisun.pet.view.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener{
	...
	private static final int DEFAULT_HEADER_VIEW_HEIGHT = 110;	// 头部文件原本的高度
	Activity activity;
	...
	public boolean onMotionDown(MotionEvent ev) {
		if(!hasRecord){
			if(homeActivity!=null&&homeActivity.listView!=null){
				listview_start_last_vision_item=homeActivity.listView.getLastVisiblePosition();
				hasRecord=true;
			}
		}
		if(homeActivity!=null&&homeActivity.listView!=null){
    ...
    int listview_start_last_vision_item;
	...
	public boolean onMotionUp(MotionEvent ev) {
		int y=(int) getY();
		LogUtil.i("scroll", "listview的y的位置："+y);
		if(listview_start_last_vision_item<getListView().getLastVisiblePosition()){
			if(homeActivity!=null)homeActivity.handler.sendEmptyMessage(HomeActivity.HIDE_BACKGROUND_CONTROL);
		}else
		if(getListView().getFirstVisiblePosition()>0){
			if(homeActivity!=null)homeActivity.handler.sendEmptyMessage(HomeActivity.HIDE_BACKGROUND_CONTROL);
		}else{
			if(homeActivity!=null)homeActivity.handler.sendEmptyMessage(HomeActivity.SHOW_BACKGROUND_CONTROL);
		}
		mIsDown = false;
		// 避免和点击事件冲突
	...
	public void setActivity(Activity activity,int mode){
